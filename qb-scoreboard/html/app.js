...
const Open = (data) => {
  ...
  $.each(data.requiredCops, (i, category) => {
    ...
    if (category.busy) $(status).html('<i class="fas fa-clock"></i>');
    else if (data.currentCops >= category.minimumPolice)
      $(status).html('<i class="fas fa-check"></i>');
    else $(status).html('<i class="fas fa-times"></i>');
  });
};
...
const Setup = (data) => {
  ...
  scoreboardHtml += `
    <div class="scoreboard-info-beam" style="background: linear-gradient(180deg,  #355771,  #467992)">
